...
class SVOCE(nn.Module):
    ...
    def load_object_concepts_feat(self):
        ...
        for en_word in self.object_concepts_classes_en:
            ...
            self.word2feat[en_word] = concept_clip_feat
        self.clip_concepts_feat = torch.stack([self.word2feat.values()]) # 600, clip_hidden_size

    
    def calculate_scores(self, list_clip_image_feat):
        BATCH_SIZE = list_clip_image_feat.size(0)
        expand_clip_concepts_feat = torch.stack(self.clip_concepts_feat).expand((BATCH_SIZE, -1, -1))
        scores = torch.bmm(
    ...
    def forward(self, batch):
        ...
        top_k_concepts_vi = [[self.en2vi[self.id2en[id]] for id in per_image_top_k] for per_image_top_k in top_k_concepts_id]
        top_k_concepts_scores = torch.gather(similarity_scores, 1, top_k_concepts_id).squeeze(-1)

        #-- Fasttext Embedding
        concepts_fasttext_feat = torch.tensor([self.fasttext_embedding(concepts) for concepts in top_k_concepts_vi]).to(self.device)
